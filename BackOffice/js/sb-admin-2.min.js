...
function ApagarCampo() {
  if (typeof localStorage !== 'undefined') {
    ...
    //var itemIndex2 = Proprietarios.findIndex(item => item.Nome === nomeRemover);
    if (itemIndex !== -1/* && itemIndex2 !== -1*/) {
      //Proprietarios.splice(itemIndex2, 1);
...
const TabelaParcerias = new Vue({
  ...
  methods: {
    verParceria(item) {
      sessionStorage.setItem("ParceriaEscolhida", JSON.stringify(item));
...
function perfil() {
  ...
  if (reserva.Estado !== "Pendente" && (reserva !== "" || reserva !== null) && buttons !== null && selPros !== null) {
...
function AceitarParceria(x) {
  ...
  if (x) {
    data["Nome"] = Parceria.nome_campo
    data["Imagem"] = "";
    data["Localizacao"] = Parceria.localizacao_campo
    data["Balnearios"] = Parceria.n_balnearios
    data["Relvado"] = Parceria.piso
    data["Outros"] = ""
    data["Preco"] = Parceria.preco_campo
    data["Bola"] = Parceria.bola
    data["Coletes"] = Parceria.coletes
...
const Parceria = new Vue({
  el: '#muitacena2',
  data: {
    item: []
  },
  mounted() {
    const dados = JSON.parse(sessionStorage.getItem('ParceriaEscolhida'));

    this.item = dados;
  }
});

const esteId = new Vue({
  el: '#esteId',
  data: {
    Reservas: 0,
    ReservasPendentes: 0
  ...
  mounted() {
    ...
    this.Reservas = dados.length;
    let porAprov = 0;
    for (let i = 0; i < dados.length; i++) {
      if (dados[i].Estado === "Pendente") {
        porAprov++
      }
    }
    this.ReservasPendentes = porAprov
  }
});

const Parcerias = new Vue({
  el: '#Parcerias',
  data: {
    Parcerias: 0,
  ...
  mounted() {
    const dados = JSON.parse(localStorage.getItem('Parcerias'));
    if (dados !== null && dados !== "") {
      this.Parcerias = dados.length;
    } else {
      this.Parcerias = 0;
    }
  }
...
function RegistarRelatorio(){
  const relatorios = JSON.parse(localStorage.getItem("Relatorios")) || []
  const dados = document.getElementsByName("dados");
  const Reserva = JSON.parse(sessionStorage.getItem("ReservaEscolhida"))
  let x = {}

  for(let i = 0; i<dados.length;i++){
      x[dados[i].id] = dados[i].value
  }
...
function ConsultarRelatorio() {
  window.location.href = "ConsultarRelatorio.html"
}
...
function perfil3() {
  ...
  const aParcerias = document.querySelector("#PedidosParc");
  const aPros = document.querySelector("#Pros");
  const btnRelat = document.querySelector("#btnRelatorio");
  const btnRelat2 = document.querySelector("#btnRelatorio2");
  var itemIndex = Relatorios.findIndex(item => item.Reserva === reserva.IdReserva);
  console.log(itemIndex)
  if(itemIndex == -1){
    btnRelat2.style.display = "none"
  }else{
    btnRelat.style.display = "none"
  }
  document.getElementById("NomePerf").innerHTML = user.Username;
  if (reserva.Estado !== "Pendente" && (reserva !== "" || reserva !== null) && buttons !== null && selPros !== null) {
    buttons.style.display = "none";
...
function perfil4() {
  ...
  Balnearios.value = Relatorios[itemIndex].Balnearios
  Bolas.value = Relatorios[itemIndex].Bolas
  Coletes.value = Relatorios[itemIndex].Coletes
  Pessoas.value = Relatorios[itemIndex].Pessoas
  Outros.value = Relatorios[itemIndex].Outros
